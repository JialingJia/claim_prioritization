...
if st.session_state['logger']:
    ...
    for item in st.session_state['logger']:
        with st.container():
            # st.write(item[1])
            # if item[1]['user_query']:
            #     st.markdown(f"""
            #         ##### {item[1]['user_query'][0]['type']}: {item[1]['user_query'][0]['query']}  
            #         """)
            # else:
            #     st.markdown(f"""
            #         ##### No query
            #         """)
            # st.write(item[0]['selected_claims'])
            for index, row in item[0]['selected_claims'].iterrows():
                claim = "claim_checkbox_" + str(next(claim_id))
                if st.checkbox(row['tweet_text'], key=claim, value=False):
                    selected_claims.append([row['tweet_text'], row['tweet_id']])
